...
class Custompage extends DataMapper
{
	...
	function custompage_model_init($from_cache = FALSE)
	{
	...
	public function get($limit = NULL, $offset = NULL)
	{
		...
		// Check if the user is allowed to see protected custompages.
	...
	public function add($data = array())
	{
		...
		if (!$this->update_custompage_db($data))
		{
			log_message('error', 'add_custompage: failed writing to database');
			return false;
		}
	...
	/**
	 * Removes custompages from database.
	...
	public function remove()
	{
		...
		if (!$this->remove_custompage_db())
		{
			log_message('error', 'remove_custompage: failed to delete database entry');
			$result[] = false;
		}
		else
			$result[] = true;
	 ...
	 * @return	boolean true on success, false on failure
	 */
	public function update_custompage_db($data = array())
	{
		if (isset($data["id"]) && $data['id'] != '') {
			$this->where("id", $data["id"])->get();
			if ($this->result_count() == 0) {
				set_notice('error', _('The custompage you wanted to edit doesn\'t exist.'));
				log_message('error', 'update_custompage_db: failed to find requested id');
				return false;
			}
		...
		if ((!isset($this->id) || $this->id == '') || (isset($old_stub) && $old_stub != $this->stub))
		{
			...
			$custompages->where('stub', $this->stub)->get();
			if ($custompages->result_count() == 0)
			{
				$found = TRUE;
			}
			...
			while (!$found)
			{
				...
				$custompages = new Custompage();
				$custompages->where('stub', $pre_stub)->get();
				if ($custompages->result_count() == 0)
				{
					$this->stub = $pre_stub;
					$found = TRUE;
		...
		if (!$success)
		{
			if (!$this->valid)
			{
				...
				log_message('error', 'update_custompage_db: failed validation');
			}
			else
			{
				set_notice('error', _('Failed saving the custompage to database for unknown reasons.'));
				log_message('error', 'update_custompage_db: failed to save');
			}
			return false;
		}
	...
	/**
	 * Removes the custompage from the database
	 *
	 * @author	Woxxy
	 * @return	object a copy of the custompages that has been deleted
	 */
	public function remove_custompage_db()
	{
		$success = $this->delete();
		if (!$success)
		{
			set_notice('error', _('The custompage couldn\'t be removed from the database for unknown reasons.'));
			log_message('error', 'remove_custompage_db: id found but entry not removed');
			return false;
		}

		// Return the custompage clone
		return $temp;
	}
	...
	/**
	 * Returns a nicely built title for a custompage
	 *
	 * @author	Woxxy
	 * @return	string the formatted title for the custompage
	 */
	...
	/**
	 * Returns the href to the custompage editing
	 *
	 * @author	Woxxy
	 * @return	string href to custompage editing
	 */
	public function edit_href()
	{
		...
		if (!$CI->tank_auth->is_allowed())
			return "";
		return site_url('/admin/blog/custompages/' . $this->stub);
	}


	/**
	 * Returns the url to the custompage editing
	 *
	 * @author	Woxxy
	 * @return	string <a> to custompage editing
	 */
...
/* Location: ./application/models/custompage.php */
